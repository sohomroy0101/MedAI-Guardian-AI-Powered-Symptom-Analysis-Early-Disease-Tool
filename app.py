...
MASTER_DIR = os.path.join(APP_DIR, "MasterData")

training = pd.read_csv(os.path.join(DATA_DIR, 'Training.csv'))
testing = pd.read_csv(os.path.join(DATA_DIR, 'Testing.csv'))
...
def getDescription():
    with open(os.path.join(MASTER_DIR, 'symptom_Description.csv')) as csv_file:
        for row in csv.reader(csv_file):
            description_list[row[0]] = row[1]

def getSeverityDict():
    with open(os.path.join(MASTER_DIR, 'symptom_severity.csv')) as csv_file:
        for row in csv.reader(csv_file):
            try:
...
def getprecautionDict():
    with open(os.path.join(MASTER_DIR, 'symptom_precaution.csv')) as csv_file:
        for row in csv.reader(csv_file):
...
symptom_synonyms = {
    ...
    "temperature": "fever", "feaver": "fever", "coughing": "cough",
    "throat pain": "sore_throat", "cold": "chills",
...
def chat():
    ...
    if step == 'welcome':
        session['step'] = 'name'
        return jsonify(reply="🤖 Welcome to MedAI Guardian!\nI'm an AI-Powered Symptom Analysis & Early Disease Prediction Tool - Made by SOHOM ROY\n👉 What is your name?")
